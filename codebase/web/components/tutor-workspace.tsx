  ...
  return (
    <aside className={`tutorPanel ${open ? "tutorOpen" : ""}`} aria-label="Trợ giảng AI">
      ...
      <div className="chatScroll">
        ...
        {stage === "answered" && selected && <article className="answerCard"><header><span className="aiMark small"><Sparkles size={14} /></span><div><small>Trợ giảng AI</small><p>{selected.answer}</p></div></header><button type="button" className="sourceToggle" aria-pressed={sourceOpen} onClick={() => { const next = !sourceOpen; setSourceOpen(next); onSourceHighlight(next); }}><span><Link2 size={14} /> {sourceOpen ? "Nguồn đang được bôi sáng" : "Kiểm tra nguồn trong bài"}</span><ChevronRight size={14} /></button><footer><button type="button" onClick={correct}><RotateCcw size={14} /> Không phải ý này</button><span><Check size={13} /> Đã xác nhận ý định</span></footer><GraphTrace stage={stage} rounds={rounds} /></article>}
